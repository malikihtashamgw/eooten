 ...
 * in a way that we have a custom data type `eooten_Customizer_Dynamic_CSS` which is capable
 * of dynamically generate the CSS out from the provided array `$css_map`.
 */
if (class_exists('WP_Customize_Setting') && !class_exists('eooten_Customizer_Dynamic_CSS')) {
	class eooten_Customizer_Dynamic_CSS extends WP_Customize_Setting {
		/**
		...
		public function get_all_css_properties() {
			return array_keys($this->css_map);
		}
		...
		public function getGoogleFonts() {
			...
			foreach ($this->get_css_map() as $css_prop_raw => $css_selectors) {
				// we get here the $css_prop and $value
				extract($this->filter_css_property($css_prop_raw));
				if ('font-family' === $css_prop) { // essentially no media query
					return $value;
				}
			}
		...
		public function render_css_save() {
			$out = $this->value();
			return $out;
		}
		...
		public function filter_css_property($css_property) {
			...
			if (self::is_filterable_string($css_property) and !empty($out['value'])) {

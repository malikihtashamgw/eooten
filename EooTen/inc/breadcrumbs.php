...
function eooten_breadcrumbs($align = 'left') {

    $showOnHome  = 1; // 1 - show breadcrumbs on the homepage, 0 - don't show
    $delimiter   = '/'; // delimiter between crumbs
    $home        = get_bloginfo('name'); // text for the 'Home' link
    $blog        = get_theme_mod('eooten_blog_title', 'Blog');
    $shop        = get_theme_mod('eooten_woocommerce_title', 'Shop');
    $forums      = get_theme_mod('eooten_bbpress_title', 'Forum');
    $showCurrent = 1; // 1 - show current post/page title in breadcrumbs, 0 - don't show
    $before      = '<li><span href="#">'; // tag before the current crumb
    $after       = '</span></li>'; // tag after the current crumb
    $output      = array();
    $class     = ['bdt-breadcrumb', 'bdt-margin-remove-top'];
    ...
    if (is_home() || is_front_page()) {
        if ($showOnHome == 1) {
            $output[] = '<ul id="tm-breadcrumb" class="'. $class .'">
            <li><a href="' . esc_url($homeLink) . '">' . esc_html($home) . '</a></li><li><span> '. esc_html($blog) . '</span></li></ul>';
        }
    } else {
        $output[] = '<ul id="tm-breadcrumb" class="'. $class .'"><li><a href="' . esc_url($homeLink) . '">' . esc_html($home) . '</a></li>';
        ...
        if (is_category()) {
            ...
            $output[] = $before . esc_html__('Category', 'eooten') . ': ' . esc_html(single_cat_title('', false)) . '' . $after;

        } elseif ( is_search() ) {
            $output[] = $before . esc_html__('Search', 'eooten') . $after;
        } elseif ( is_day() ) {
            ...
            $output[] = $before . esc_html__('Tag', 'eooten') . ': ' . esc_html(single_tag_title('', false)) . $after;
        } elseif ( is_author() ) {
            global $author;
            $userdata = get_userdata($author);
            $output[] = $before . esc_html__('Articles by', 'eooten') . ' ' . esc_html($userdata->display_name) . $after;
        } elseif ( is_404() ) {
            $output[] = $before . esc_html__('Error 404', 'eooten') . $after;
        }
        if ( get_query_var('paged') ) {
            $output[] = ' (' . esc_html__('Page', 'eooten') . ' ' . esc_html(get_query_var('paged')) . ')';
        }
        $output[] = '</ul>';
    }
